...
def create_video(quote, background_video, output_file):
    ...
    text = TextClip(wrapped_quote, fontsize=24, color='white', font='Arial').set_duration(text_duration).fadein(1)

    # Overlay the text on the video
    result = CompositeVideoClip([video, text.set_position(("center", video.h -400))])
    ...
    check_bio = TextClip("Sprawdź w Bio", fontsize=30, color="white", font="Arial").set_duration(text_duration).set_position(("center", logo_position[1] - 200)).fadein(1)
